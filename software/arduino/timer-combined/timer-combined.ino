/*================================================================================*
   ...
   Combined Arduino Uno R3 / Uno R4 Minima build.
   Board is detected at compile time:
     - ARDUINO_UNOR4_MINIMA  -> Arduino Uno R4 Minima (RA4M1)
     - all other (default)   -> Arduino Uno R3 (AVR ATmega328P)
 *================================================================================*/
...
#if defined(ARDUINO_UNOR4_MINIMA)
    // R4 Minima: read all lane pins from Port 1 in a single register access
    uint16_t port1 = R_PORT1->PIDR;
    for (int n=0; n<NUM_LANES; n++) lane_status[n] = (port1 >> LANE_DET_BIT[n]) & 1;
#else
    // R3 (AVR): read lane pins from Port D via PIND register
    for (int n=0; n<NUM_LANES; n++) lane_status[n] = bitRead(PIND, LANE_DET[n]);
#endif
...
#if defined(ARDUINO_UNOR4_MINIMA)
      uint16_t port1 = R_PORT1->PIDR;
      lane_status[n] = (port1 >> LANE_DET_BIT[n]) & 1;
#else
...
#if defined(ARDUINO_UNOR4_MINIMA)
  Serial.println("  BOARD         Arduino Uno R4 Minima");
#else
  Serial.println("  BOARD         Arduino Uno R3");
#endif
